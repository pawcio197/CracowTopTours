...
{
    register_post_type('Wycieczki', array(
        'label' => 'Wycieczki',
        'labels' => array(
            'name' => 'Wycieczki',
            'menu_name' => 'Wycieczki',
            ...
            'view_item' => 'Pokaż wycieczki',
            'search_items' => 'Szukaj wycieczki',
            'not_found' => 'Nie odnaleziono wycieczki',
            'not_found_in_trash' => 'Nie odnaleziono wycieczki w koszu'
        ),
        'hierarchical' => false,
        'public' => false,
...
function tours_func()
{
    ...
    foreach ($tours as $item):?>
        <div class="item-tours">
            <div class="image-item-tours">
                <img src="<?php echo get_the_post_thumbnail_url($item->ID); ?>">
            </div>
            <div class="text-item-tours">
                <span class="title-item-tours"><?php echo $item->post_title; ?></span>
                <span class="post-content-item-tours"><?php echo wp_trim_words($item->post_content, 55) ?></span>
                <a href="http://<?php echo get_post_meta($item->ID, 'link-tours')[0]; ?>" target="_blank">More information</a>
            </div>
...
add_shortcode('Wycieczki', 'tours_func');

function udi_get_meta($value)
{
    global $post;
...
add_action('add_meta_boxes', 'link_tours');

function classy_html($post)
{
    wp_nonce_field('_classy_nonce', 'classy_nonce'); ?>
    <p>
    <input type="text" name="link-tours" id="link-tours" value="<?php echo udi_get_meta('link-tours'); ?>">
    </p><?php
}

function classy_save($post_id)
{
    if (defined('DOING_AUTOSAVE') && DOING_AUTOSAVE) return;
    if (!isset($_POST['classy_nonce']) || !wp_verify_nonce($_POST['classy_nonce'], '_classy_nonce')) return;
    if (!current_user_can('edit_post', $post_id)) return;

    if (isset($_POST['link-tours']))
        update_post_meta($post_id, 'link-tours', esc_attr($_POST['link-tours']));
}

add_action('save_post', 'classy_save');
...
function create_opinie()
{
    register_post_type('Opinie', array(
        ...
        'labels' => array(
            ...
            'view_item' => 'Pokaż opinie',
            'search_items' => 'Szukaj opinii',
            'not_found' => 'Nie odnaleziono opinii',
            'not_found_in_trash' => 'Nie odnaleziono opinii w koszu'
        ),
        'hierarchical' => false,
        'public' => false,
        'exclude_from_search' => true,
...
add_action('init', 'create_opinie', 0);
...
function reviews_func()
{
    ...
    ?>
    <div class="owl-carousel-theme owl-theme"><?php
        foreach ($reviews as $item):?>
            <div class="item_owl">
                <div class="reviews-text">
                    <?php echo wp_trim_words($item->post_content, 1000) ?>
                </div>
                <div class="reviews-title">
                    <?php echo $item->post_title; ?>
                </div>
                <span class="up-reviews">Reviews about us!</span>
            </div>
        <?php endforeach; ?>
    </div>
    <?php return ob_get_clean();
...
add_shortcode('Opinie', 'reviews_func');